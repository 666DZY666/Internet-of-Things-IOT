...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagdata)
    {
        info->event[info->num] = EVENT_DATA;
        info->num++;
        memcpy((uint8_t *)dataPoints->valuedata,issuedData->attrVals.valuedata,sizeof(issuedData->attrVals.valuedata));
    }
...
//    static uint32_t lastReportTime = 0;
    ...
    if(0 != memcmp((uint8_t *)&last->valuedata,(uint8_t *)&cur->valuedata,sizeof(last->valuedata)))
    {
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    memcpy((uint8_t *)devStatusPtr->valuedata,(uint8_t *)&dataPoints->valuedata,sizeof(dataPoints->valuedata));
    return 0;
}

...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagbutton)
    {
        info->event[info->num] = EVENT_button;
        info->num++;
        dataPoints->valuebutton = gizDecompressionValue(button_BYTEOFFSET,button_BITOFFSET,button_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
		if(0x01 == issuedData->attrFlags.flaglight)
    {
        info->event[info->num] = EVENT_light;
        info->num++;
        dataPoints->valuelight = gizX2Y(light_RATIO,  light_ADDITION, issuedData->attrVals.valuelight); 
    }
    return 0;
}
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizCompressValue(button_BYTEOFFSET,button_BITOFFSET,button_LEN,(uint8_t *)devStatusPtr,dataPoints->valuebutton);
